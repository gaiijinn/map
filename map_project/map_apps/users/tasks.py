...
#
#     if user:
#         total_exp = (AchievementsProgressStatus.objects.select_related('achievement')
#                      .filter(user=user, is_achieved=True)
#                      .aggregate(total_exp=Sum('achievement__given_exp'))['total_exp'] or 0)
#
#         user_level = UserLevel.objects.filter(low_range__lte=total_exp, top_range__gte=total_exp).first()
#
#         if user_level:
#             UserProfile.objects.filter(user=user).update(user_level=user_level)
...
def level_calculating(user_id: int):
    ...
    if user:
        ...
        current_level = UserLevel.objects.filter(low_range__lte=total_exp, top_range__gte=total_exp).first()
